...
- **Конфигурация**:
  - `top_k_docs` - количество извлекаемых документов (по умолчанию 5)
  - `search_type` - тип поиска (similarity)
...
```
.
├── rag_backend/          # FastAPI бэкенд с RAG логикой
│   ├── src/
│   │   ├── modules/rag/
│   │   │   ├── routers/    # API endpoints
│   │   │   ├── components/ # RAG, LLM, Embeddings
│   │   │   └── schemas.py  # Pydantic схемы
│   │   ├── common/
│   │   │   ├── database.py # История чата (TinyDB)
│   │   │   └── qdrant/     # Векторная БД
...
│   ├── chat_handler.py  # Логика обработки чата
│   ├── ui_components.py # UI компоненты
│   ├── Dockerfile
│   └── requirements.txt
│
├── evaluation/          # Тестирование и валидация
...
Простой веб-интерфейс на Streamlit для чат-бота по книге "Преступление и наказание".

### Возможности

- Чат-интерфейс в стиле современных мессенджеров
- Отображение истории переписки
- Показ источников (главы книги) в раскрывающихся блоках
- Примеры вопросов в боковой панели
- Кнопка очистки истории чата

### Модули фронтенда

**app.py** - главная точка входа приложения. Инициализирует Streamlit, создает UI и управляет потоком выполнения.

**config.py** - конфигурация приложения (URL API, настройки страницы, примеры вопросов).

**api_client.py** - клиент для общения с FastAPI бэкендом. Инкапсулирует логику HTTP-запросов.

**chat_handler.py** - обработчик логики чата (управление состоянием сообщений, обработка вопросов, взаимодействие с API).

**ui_components.py** - переиспользуемые UI компоненты (боковая панель, отображение сообщений и источников).
...
**Валидационные данные**: "Преступление и наказание" - 50 вопросов с эталонными ответами.
...
Для каждого из 50 вопросов из валидационного датасета:
...
# Запуск оценки на всех 50 вопросах
...
- **Contextual Recall (0.745)**. Извлекается достаточный контекст для ответа (можно улучшить)
